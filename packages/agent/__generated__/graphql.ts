...
export type GetPoolsQuery = { __typename?: 'Query', pools: Array<{ __typename?: 'Pool', id: string, feeTier: string, totalValueLockedUSD: string, volumeUSD: string, txCount: string, token0: { __typename?: 'Token', symbol: string }, token1: { __typename?: 'Token', symbol: string } }> };
...
export const GetPoolsDocument = gql`
  ...
  pools(
    ...
    orderDirection: desc
    where: {or: [{token0_: {symbol: $token0}, token1_: {symbol: $token1}}, {token0_: {symbol: $token1}, token1_: {symbol: $token0}}]}
  ) {
    id
    token0 {
      symbol
...
export function getSdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
  return {
    GetPools(variables: GetPoolsQueryVariables, requestHeaders?: GraphQLClientRequestHeaders, signal?: RequestInit['signal']): Promise<GetPoolsQuery> {
      return withWrapper((wrappedRequestHeaders) => client.request<GetPoolsQuery>({ document: GetPoolsDocument, variables, requestHeaders: { ...requestHeaders, ...wrappedRequestHeaders }, signal }), 'GetPools', 'query', variables);
    }
  };
}
